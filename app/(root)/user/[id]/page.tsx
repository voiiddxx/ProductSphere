   ...
   <DashBoardNav/>
    ...
    <div className='w-full flex justify-between' >
        ...
        <div className='h-[150px] w-[150px] rounded-full bg-white' >
            <Image src={user.data.avatar} className='h-[150px] w-[150px] object-cover' height={1500} width={1500} alt='userimaeg' />
        </div>
        <div>
            <h1 className='text-zinc-800 text-4xl font-semibold mt-2' >{user.data.username}</h1>
           <div className='flex gap-2' >
           <p className='text-xs font-medium bg-slate-200 px-2 py-1 rounded-full mt-1' >{user.data.email}</p>
           </div>
            ...
            <div className='flex gap-8 flex-wrap  mt-8' >
                {
                    user.data.ownedProducts.map((curr:any)=>{
                        return  <Link href={`/user.data/${curr.id}`} >
                        <div className="h-[350px] w-[350px] bg-white rounded-md shadow-[0px_20px_20px_10px_#edf2f7]">
                          ...
                          <div className="mt-10 px-6" >
                             <div className='flex gap-2' >
                             <h1 className="text-lg font-semibold text-zinc-950" >{curr.name}</h1>
                             <div className='bg-yellow-100 flex justify-center text-yellow-700 items-center rounded-full px-2 py-1' >
                                ...
                                <p className='text-xs font-medium' >{curr.visibility}</p>
                             </div>
                             </div>
                              <p className="mt-2 text-sm font-medium text-zinc-600" >{curr.detail}</p>
